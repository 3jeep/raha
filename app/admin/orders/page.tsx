...
export default function AdminOrdersPage() {
  ...
  const fetchLastCrew = async (userId: string) => {
    ...
    const q = query(collection(db, "bookings"), where("userId", "==", userId), where("status", "in", ["completed", "completed_for_today"]), orderBy("actualFinishedAt", "desc"), limit(1));
  ...
  useEffect(() => {
    ...
    const unsubB = onSnapshot(query(collection(db, "bookings"), orderBy("createdAt", "desc")), async (snap) => {
      const rawOrders = await Promise.all(snap.docs.map(async (d) => {
      ...
      const filtered = rawOrders.filter(order => {
        ...
        const isMonthly = type === "monthly_contract";
        ...
        if (type !== "monthly_contract") {
            ...
            // منطق الـ 30 يوم من أول زيارة فعلية
            if (order.contractStartDate) {
                const startMs = order.contractStartDate.seconds * 1000;
                const thirtyDaysMs = 30 * 24 * 60 * 60 * 1000;
                if (Date.now() > (startMs + thirtyDaysMs)) {
                    if (order.status !== "contract_finished") handleUpdate("bookings", order.id, { status: "contract_finished" });
                    return false;
                }
            }
            // يظهر العقد إذا كان اليوم ضمن المختار، ولا نخفيه إذا كانت الحالة completed_for_today للسماح ببدء زيارة اليوم التالي
            return order.selectedDays?.includes(selectedDayName) && order.status !== "contract_finished";
        }
      });
            ...
            <div key={b.id} className={`bg-white rounded-[40px] shadow-xl p-7 border-2 relative ${(b.type === 'monthly_contract' || b.category === 'monthly_contract') ? 'border-blue-100' : 'border-gray-50'}`}>
              <div className="absolute top-6 left-6 flex gap-2">
                {b.status === "in-progress" && (
                  <button onClick={() => handleUpdate("bookings", b.id, {status:"pending"})} className="bg-orange-100 text-orange-600 p-2 rounded-xl text-[9px] font-black italic">🛑 إيقاف</button>
                )}
                <button onClick={() => handleDelete("bookings", b.id)} className="bg-red-50 text-red-500 p-2 rounded-xl text-[9px] font-black italic">🗑️ حذف</button>
              </div>
              ...
              <div className="mb-6">
                ...
                <h4 className="font-black text-xl text-gray-800">{b.fullName}</h4>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                  <select value={b.assignedMaid || b.suggestedMaid || ""} onChange={(e) => { const m = sortedMaids.find(x => x.name === e.target.value); autoSave(b.id, { assignedMaid: e.target.value, maidDist: m?.dist?.toFixed(2) }); }} className="p-3 rounded-2xl text-[9px] font-black bg-gray-50 border-none outline-none">
                  ...
                  <select value={b.assignedVehicle || b.suggestedVehicle || ""} onChange={(e) => { const v = sortedVehs.find(x => x.driverName === e.target.value); autoSave(b.id, { assignedVehicle: e.target.value, driverDist: v?.dist?.toFixed(2) }); }} className="p-3 rounded-2xl text-[9px] font-black bg-gray-50 border-none outline-none">
              ...
              {b.status === "in-progress" ? (
                ...
                <button onClick={async () => { 
                  ...
                  if (b.type === "monthly_contract" && !b.contractStartDate) updates.contractStartDate = serverTimestamp();
                  await handleUpdate("bookings", b.id, updates);
                  showToast("تم بدء المهمة 🚀");
                }} className="w-full bg-[#1E293B] text-white py-5 rounded-3xl font-black text-xs">{(b.status === "completed_for_today") ? "بدء الزيارة التالية 🔄" : "تـسـجـيـل الـبـدء 🚀"}</button>
              )}
            </div>
          );
        })}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[40px] p-6 space-y-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <button onClick={async () => {
                ...
                // --- الإضافة المرجعية: حفظ نسخة الزيارة في Sub-collection ---
                if (isMonthly) {
                    await addDoc(collection(db, "bookings", selectedOrder.id, "visits"), {
                ...
                setShowModal(false);
                showToast(isMonthly ? "تم أرشفة الزيارة في السجل المرجعي ✅" : "تم الإتمام بنجاح ✅");
